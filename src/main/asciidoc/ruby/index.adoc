...
----
compile 'io.vertx:vertx-jgroups:3.3.0-SNAPSHOT'
----


If you are using Vert.x from the command line, the jar corresponding to this cluster manager (it will be named
`vertx-jgroups-3.3.0-SNAPSHOT.jar` should be in the `lib` directory of the Vert.x installation. Don't forget to
...
----
require 'vertx/vertx'
mgr = Java::IoVertxSpiClusterJgroups::JGroupsClusterManager.new()
options = {
  'clusterManager' => mgr
}
...
You can instantiate `link:unavailable[JGroupsClusterManager]` with your own instance of
`JChannel`:

[source,ruby]
----
require 'vertx/vertx'
mgr = Java::IoVertxSpiClusterJgroups::JGroupsClusterManager.new(channel)
options = {
  'clusterManager' => mgr
}
Vertx::Vertx.clustered_vertx(options) { |res_err,res|
